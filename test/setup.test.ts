 ...
 *
 * Two MCP servers (connector, docs) × four clients (claude-code,
 * cursor, windsurf, vscode) × default-both-with-opt-out flags.
 *
...
describe("CLIENTS registry", () => {
  it("includes claude-code and vscode as CLI-driven clients", () => {
    const cli = Object.entries(CLIENTS).filter(([, c]) => c.kind === "cli");
    expect(cli.map(([k]) => k).sort()).toEqual(["claude-code", "vscode"]);
