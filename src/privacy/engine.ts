...
const BOOSTER_KEYWORDS: Record<string, RegExp> = {
  ...
  // PASSPORT keywords expanded per docs/research/pii-passport-format-landscape.md.
  // Real passport-bearing content reliably contains one of these within ±48
  // chars; URL hex fragments / ISO committee codes / git SHAs / hex colours
  // do not. This is the cleanest discriminator.
...
export function scanString(
  ...
  for (let i = 0; i < rs.rules.length; i++) {
    ...
    while ((m = rule.regex.exec(input)) !== null) {
      ...
      // Tier policy (docs/pii-tier-policy.md): low-confidence rules require
      // at least one of their declared context boosters to match within the
      // BOOSTER_WINDOW. This generalises the hardcoded context-gates above
      // (ADDRESS, PHONE_FALLBACK, DOB) so any rule can be gated by tagging
      // confidence: low + context_boosters in the ruleset JSON. Rules with
      // confidence: low and zero context boosters retain their pre-policy
      // behaviour (shape match + optional validator) — adopt those onto the
 ...
 * for confidence: low rules per docs/pii-tier-policy.md. Mirrors the window
 * sizing and lookup pattern already used by computeContextBoost so the policy
 * gate and the priority bump see the same context window.
 */
