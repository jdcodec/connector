 ...
 *
 * Four targets are supported today, in two flavours.
 *
 * The target name is what the customer types — it does *not* always
 * map 1:1 with the AI agent the customer is using. Two of the
 * targets reach more agents than their name implies:
 *
 *   - `claude-code` writes to `~/.claude.json` via `claude mcp add`.
 *     That config file is read by both the Claude Code CLI (terminal)
 *     AND Anthropic's Claude VS Code extension. So this target wires
 *     JD Codec into both. Customers running the Claude extension
 *     inside VS Code should pick this target — NOT `vscode` below.
 *   - `vscode` writes to VS Code's own `mcp.json` via `code --add-mcp`.
 *     That config is read by VS Code's native Copilot agent (and any
 *     other extension that consumes VS Code's MCP config). It is NOT
 *     read by the Claude VS Code extension.
 *
 *   CLI clients (auto-execute, fall back to printed commands):
 *   - `claude-code` — `claude mcp add ...`
 *                     also covers Anthropic's Claude VS Code extension
 *                     (same `~/.claude.json` config file).
 *   - `claude-vscode` — alias of `claude-code`. Same flow, same
 *                       config destination. Provided so customers
 *                       running the Claude VS Code extension can
 *                       reach for the name they think in.
 *   - `vscode`      — `code --add-mcp '{json}'`
 *                     covers VS Code's native Copilot agent + other
 *                     extensions reading VS Code's `mcp.json`. Does
 *                     NOT cover the Claude extension; that uses
 *                     `claude-code` (or `claude-vscode`) above.
 *
...
// Canonical spec for the Claude target. Reused (not duplicated) for the
// `claude-vscode` alias below — same config file (`~/.claude.json`),
// same `claude mcp add` invocation, same flow.
const CLAUDE_SPEC: ClientSpec = {
  kind: "cli",
  // Customer-visible name. Includes the Claude VS Code extension
  // because that extension shares ~/.claude.json with the CLI — both
  // reach this target.
  name: "Claude Code (CLI + Claude VS Code extension)",
  cmd: "claude",
...
export const CLIENTS: Record<string, ClientSpec> = {
  // Canonical key. `claude-vscode` below is an alias pointing at the
  // same spec — both invocations are equivalent in flow + outcome.
  "claude-code": CLAUDE_SPEC,
  // Alias for users who think of themselves as "running Claude in
  // VS Code". The Claude VS Code extension reads ~/.claude.json (same
  // file the CLI uses), so a single `claude mcp add` registration
  // covers both. Same code path; renders alongside `claude-code` in
  // help with a "(alias)" marker so the equivalence is explicit.
  "claude-vscode": CLAUDE_SPEC,
  cursor: {
    kind: "manual",
    name: "Cursor",
  ...
  vscode: {
    kind: "cli",
    // Customer-visible name. Disambiguates from the Claude VS Code
    // extension, which lives behind `claude-code` (different config
    // file). This target wires VS Code's native Copilot agent.
    name: "VS Code (Copilot agent — not the Claude extension)",
    cmd: "code",
    installUrl:
...
// Aliases — keys that resolve to the same ClientSpec as another key.
// Listed here so help output can mark them as aliases instead of
// presenting them as independent targets. Resolution of the alias
// happens implicitly via shared object identity in CLIENTS itself
// (e.g. CLIENTS["claude-vscode"] === CLIENTS["claude-code"]).
...
export function printSetupHelp(display: DisplayIO = defaultDisplay): void {
  ...
  for (const key of CLIENT_KEYS) {
    ...
    const aliasOf = CLIENT_ALIASES[key];
    const aliasMarker = aliasOf ? palette.dim(` (alias of ${aliasOf})`) : "";
    display.print(`  ${palette.info(key.padEnd(14))}${c.name} ${palette.dim(note)}${aliasMarker}`);
  }
  display.print("");
  ...
  display.print(`  ${palette.bold("jdcodec setup claude-code")}              wire both into Claude Code (CLI + Claude VS Code extension)`);
  display.print(`  ${palette.bold("jdcodec setup vscode")}                   wire both into VS Code (Copilot agent — not the Claude extension)`);
  ...
  display.print("");
  display.print(` ${palette.dim("Using Claude inside VS Code? Pick claude-code (or its alias claude-vscode), not vscode — they reach different config files.")}`);
  display.print("");
}
